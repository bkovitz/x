...
(define (watch-graph-mixin %)
  (class % (super-new)
    (define after-move-to (lambda (snip x y dragging?)
      (unless dragging? (displayln `(,snip ,x ,y)))))
    (augment after-move-to)))

;; the graph editor does all the magic:
(define graph-editor% (watch-graph-mixin (graph-pasteboard-mixin pasteboard%)))
(define graph-ed
    (new graph-editor%
